...
|_ -> raise X_this_should_not_happen;;
 ...
 |TagRef(ref_str)-> let is_exist = List.fold_right (fun curr_str acc -> 
 if(curr_str = ref_str) then true else acc) tagsList.listtags false in 
 if(is_exist = false) then let () = printf("at tagRef ") in raise X_this_should_not_happen else sexp 
 ...
 let () = printf("add at taggedSexpr ") in  TaggedSexpr(ref_str, checkTaggedRef nsexp)
